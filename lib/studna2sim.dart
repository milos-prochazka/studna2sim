...
Future studnaMqtt(
    {required String server, required String token, dynamic config}) async {
  final mode = stringToEnum(StudnaDeviceMode.values,
      getString(getItemFromPath(config, ['variant']), defaultValue: 'single'));
  final upSpeed =
      getDouble(getItemFromPath(config, ['up_speed']), defaultValue: 0.02);
  final downSpeed =
      getDouble(getItemFromPath(config, ['down_speed']), defaultValue: 0.005);
  final device = StudnaDevice(
      server: server,
      token: token,
      mode: mode,
      upSpeed: upSpeed,
...
class StudnaDevice extends ThingsboardDevice {
  ...
  void publish() {
    ...
    final dout1ManualOverride =
        _dout1Mode.mode != _StudnaOutputModeEnum.manual && _manualDout1Override;
    final dout1Mode =
        dout1ManualOverride ? _StudnaOutputModeEnum.manual : _dout1Mode.mode;

    final dout2ManualOverride =
        _dout2Mode.mode != _StudnaOutputModeEnum.manual && _manualDout2Override;
    final dout2Mode =
        dout2ManualOverride ? _StudnaOutputModeEnum.manual : _dout2Mode.mode;

    final telemetry = {
      "version": 2,
      if (hasAin1)
        "ain1": {
          "str": ain1.toStringAsFixed(2),
          "units": "m",
          "zone": ain1Zone.name
        },
      ...
      if (hasCnt1)
        "cnt1": {"value": cnt1, "past_value": cnt1Past, "units": "l"},
      ...
      if (hasBattery)
        "power": {"battery_charge": 100, "power_supply": "battery"},
  ...
  double get weekSecond {
    ...
    return 86400.0 * (now.weekday - DateTime.monday) +
        now.hour * 3600.0 +
        now.minute * 60.0 +
        now.second;
  }
  ...
  double get weekMinute {
    final now = DateTime.now();
    return 1440.0 * (now.weekday - DateTime.monday) +
        now.hour * 60.0 +
        now.minute;
  }
      ...
      case 'full_config':
        {
          if (value is Map) {
            ...
            name = getString(getItemFromPath(system, ['station_name']),
                defaultValue: name);
            updateInterval = getDouble(
                getItemFromPath(system, ['update_interval']),
                defaultValue: 60.0);
  ...
  void _state() {
    ...
    if (DateTime.now().difference(lastTelemetryTime).inSeconds >=
        updateInterval) {
      publishRequest = true;
    }
  ...
  void _control() {
    StudnaAinZone controlAinLevel(
        StudnaAinZone currentLevel, double value, _StudnaAinMode mode) {
    ...
    double controlAin(
        double level, bool output, bool hasDout, _StudnaOutputMode outputMode) {
      if (!hasDout) {
        level += solarSpeed;
        if (--solarTimer <= 0) {
              ...
              if (scheduler != null) {
                for (int day = 0; day < 7; day++) {
                  if (scheduler.days & (1 << day) != 0) {
                    begin = dayMinutes * day +
                        hourMinutes * scheduler.hours +
                        scheduler.minutes;
    ...
    bool testManualEnd(DateTime manualEnd, double level, double manualStart,
        _StudnaOutputMode outputMode) {
    ...
    if (hasDout1) {
      _manualDout1Override = _manualDout1Override &&
          !testManualEnd(_manualDout1End, ain1, _manualOut1Start, _dout1Mode);
      _setDout1(controlDout(
          output: dout1,
          zone: ain1Zone,
          level: ain1,
          outputMode: _dout1Mode,
          manualOverride: _manualDout1Override));
    }

    if (hasDout2) {
      _manualDout2Override = _manualDout2Override &&
          !testManualEnd(_manualDout2End, ain2, _manualOut2Start, _dout2Mode);
      _setDout2(controlDout(
          output: dout2,
          zone: ain2Zone,
          level: ain2,
          outputMode: _dout2Mode,
          manualOverride: _manualDout2Override));
    }
      ...
      case 'setdout1':
        {
          _setDout1(params);
          _manualDout1End =
              DateTime.now().add(Duration(seconds: _dout1Mode.maxTime.toInt()));
      ...
      case 'setdout2':
        {
          _setDout2(params);
          _manualDout2End =
              DateTime.now().add(Duration(seconds: _dout2Mode.maxTime.toInt()));
...
class _StudnaOutputMode {
  ...
  _StudnaOutputMode(String name, dynamic config)
      : name = getString(getItemFromPath(config, ['name']), defaultValue: name),
        mode = stringToEnum(
            _StudnaOutputModeEnum.values,
            getString(getItemFromPath(config, ['mode']),
                defaultValue: 'manual')),
        minimum =
            getDouble(getItemFromPath(config, ['minimum']), defaultValue: 0.0),
        maximum =
            getDouble(getItemFromPath(config, ['maximum']), defaultValue: 5.0),
        maxLevelChange = getDouble(
            getItemFromPath(config, ['max_level_change']),
            defaultValue: 1.0),
        maxTime = 60.0 *
            getDouble(getItemFromPath(config, ['max_time']),
                defaultValue: 60.0),
        schedulerOkHigh =
            decodeScheduler(getItemFromPath(config, ['scheduler', 'ok_high'])),
        schedulerCriticalLow = decodeScheduler(
            getItemFromPath(config, ['scheduler', 'critical_low']));
...
class _StudnaAinMode {
  ...
  final levelLimits = <(double min, double max)>[
    (-999999.9, 1.6),
    (1.5, 2.1),
    (2.0, 4.5),
    (4.4, 999999.9)
  ...
  _StudnaAinMode(String name, dynamic config)
      : name = getString(getItemFromPath(config, ['name']), defaultValue: name),
        units =
            getString(getItemFromPath(config, ['units']), defaultValue: 'm'),
        hysteresis = getDouble(getItemFromPath(config, ['hysteresis']),
            defaultValue: 0.1) {
